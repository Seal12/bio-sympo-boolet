# Custom domain: symposium-booklet.vividhouse.co.za

This site is hosted on **GitHub Pages** and served at:

**https://symposium-booklet.vividhouse.co.za/**

The domain `vividhouse.co.za` is registered with Afrihost. The subdomain `symposium-booklet` points at the same GitHub Pages deployment as before.
...
- `public/CNAME` — contains `symposium-booklet.vividhouse.co.za` (copied into `out/` on build)
...
Use only the subdomain label (`symposium-booklet`), not the full hostname, if Afrihost asks for the host field.
...
3. Enter: `symposium-booklet.vividhouse.co.za`
4. Wait until GitHub shows the domain as verified.
5. Enable **Enforce HTTPS**.

### 3. Verify

Open **https://symposium-booklet.vividhouse.co.za/** and confirm pages and images load correctly.
